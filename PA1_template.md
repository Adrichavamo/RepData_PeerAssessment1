...
dataset. In this part, we use the dataset with the filled-in values.
    ...
    # This set because my country is Costa Rica, and i need validate the weekday in english (for standards purposes) 
    ...
    filled.data$dayType <- sapply(filled.data$date, function(x) {
    ...
    activityByDay <-  aggregate(steps ~ interval + dayType, filled.data, mean, na.rm = TRUE)
